...
class CustomButton: UIButton {
    ...
    required init(coder aDecoder:NSCoder){
         ...
         if(self.tag == 10)
        {
          ...
          self.titleLabel?.font=UIFont(name: Constant.sharedinstance.Regular, size: (self.titleLabel?.font.pointSize)!)
        }
        else if(self.tag == 11)
        {
            self.titleLabel?.font=UIFont(name: Constant.sharedinstance.Bold, size: (self.titleLabel?.font.pointSize)!)
        }
        else if(self.tag == 13)
        {
            self.titleLabel?.font=UIFont(name: Constant.sharedinstance.SemiBold, size: (self.titleLabel?.font.pointSize)!)
        }
        else if(self.tag == 14)
        {
            self.titleLabel?.font=UIFont(name: Constant.sharedinstance.Medium, size: (self.titleLabel?.font.pointSize)!)
        }

        else if(self.tag == 15)
        {
            self.titleLabel?.font=UIFont(name: Constant.sharedinstance.Light, size: (self.titleLabel?.font.pointSize)!)
        }
    ...
    @IBInspectable var isAddTextColor: Bool = false {
        didSet {
            if(isAddTextColor)
            {
                self.setTitleColor(Themes.sharedInstance.returnThemeColor(), for: .normal)
            }
        }
    }
